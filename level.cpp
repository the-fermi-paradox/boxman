...
GameObject &Level::getPlayer() { return player; }
Level::Level(const std::filesystem::path &path, const SpriteSheet &ss)
{
    ...
    for (size_t i = 0; i < level_data.size(); ++i) {
        for (size_t j = 0; j < level_data[i].size(); ++j) {
            ...
            auto floor =
                    GameObject("ground_01", j * TILE_WIDTH, i * TILE_HEIGHT,
                               ss.GetSpriteWidth("ground_01"),
                               ss.GetSpriteHeight("ground_01"));
            auto g = GameObject(s, j * TILE_WIDTH, i * TILE_HEIGHT,
    ...
    player = GameObject("player_01", player_start_x * TILE_WIDTH,
                        player_start_y * TILE_HEIGHT,
                        ss.GetSpriteWidth("player_01"),
                        ss.GetSpriteHeight("player_01"), true);
