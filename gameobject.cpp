...
std::string GameObject::getType() const { return type; }
...
bool GameObject::checkCollision(const GameObject &B) const
{
    if (!B.collidable)
        return false;
    auto &A = *this;
    ...
    /* The -30 here is to handle the isometric perspective and make things feel
     * more natural. */
...
bool GameObject::checkCollision(State &state) const
{
    for (auto &object: state.statics) {
        if (&object != this && this->checkCollision(object))
            return true;
    }
...
int GameObject::getCategory() const
{
    if (this->type == "ground_01") {
        return 0;
    } else if (this->type == "environment_01" || this->type == "block_03" ||
               this->type == "block_01") {
        return 1;
